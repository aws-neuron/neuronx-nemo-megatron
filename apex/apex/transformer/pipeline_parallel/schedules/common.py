...
def build_model(
    ...
    **kwargs: Any,
    ...
    def copy_tensor_model_parallel_attributes_to_dict(
        ...
        def maybe_copy(attribute):
            if hasattr(source_tensor, attribute):
                temp_dict[attribute] = getattr(source_tensor, attribute)
    ...
    def copy_tensor_model_parallel_attributes_from_dict(
        destination_tensor: torch.Tensor, source_dict: dict
    ) -> None:
        for attribute in _MODEL_PARALLEL_ATTRIBUTE_DEFAULTS:
            setattr(destination_tensor, attribute, source_dict[attribute])

    for model_module in model:
        # model_module.cuda(torch.cuda.current_device())
        tensor_param_dict = {}
        for name, param in model_module.named_parameters(recurse=True):
            tensor_param_dict[name] = copy_tensor_model_parallel_attributes_to_dict(
                param
            )
...
def forward_step(
    ...
    # Neuron: Always split forward and backward graphs, even if pipeline parallel isn't enabled. 
