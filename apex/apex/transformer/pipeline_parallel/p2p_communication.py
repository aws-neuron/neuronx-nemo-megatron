...
def _communicate(
    ...
    *,
    ...
    import torch_xla.core.xla_model as xm
    rank = xm.get_ordinal()
    tensor_send_prev_out = None
    tensor_send_next_out = None
    tensor_recv_prev_out = None
    tensor_recv_next_out = None
    if tensor_recv_prev is not None:
        tensor_recv_prev = xm.all_reduce(xm.REDUCE_SUM, tensor_recv_prev,
                                groups=parallel_state.get_prev_rank_group())
    if tensor_send_prev is not None:
        _ = xm.all_reduce(xm.REDUCE_SUM, tensor_send_prev,
                                groups=parallel_state.get_prev_rank_group())
    if tensor_send_next is not None:
        _ = xm.all_reduce(xm.REDUCE_SUM, tensor_send_next,
                                groups=parallel_state.get_next_rank_group())
