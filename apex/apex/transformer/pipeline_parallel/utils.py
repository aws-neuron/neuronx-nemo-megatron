...
def get_kth_microbatch(batch: Optional[List[torch.Tensor]], k: int) -> List[torch.Tensor]:
    ...
    for x in batch:
        ...
        microbatch.append(torch.stack(x[start:end]))
    assert len(microbatch) > 0, "Microbatch lenght less than 0"
    return microbatch


def get_batch_iterator(batch: Optional[List[torch.Tensor]]):
    """Create a list of microbatches from a list of local minibatches.
    ...
    if batch is None:
        all_batches = [torch.zeros(1, device=xm.xla_device())]*num_micro_batches
        return MpDeviceLoader(
            all_batches, xm.xla_device(),
            batches_per_execution = get_num_microbatches() if parallel_state.get_pipeline_model_parallel_world_size() > 1 else 1 )
    ...
    return MpDeviceLoader(all_batches, xm.xla_device(), batches_per_execution = get_num_microbatches() if parallel_state.get_pipeline_model_parallel_world_size() > 1 else 1 )
...
def get_ltor_masks_and_position_ids(
    ...
    # Position ids.
    position_ids = torch.arange(seq_length, dtype=torch.int, device=data.device)
