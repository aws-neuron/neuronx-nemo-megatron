...
class XlaRNGStatesTracker:
    ...
    def add(self, name, seed):
        ...
        import torch_xla.core.xla_model as xm
        orig_rng_state = xm.get_rng_state()
        # Set the new state and store it.
        xm.set_rng_state(seed)
        self.states_[name] = xm.get_rng_state()
        # Reset rng state to what it was.
        xm.set_rng_state(orig_rng_state)
    ...
    def fork(self, name=_MODEL_PARALLEL_RNG_TRACKER_NAME):
        ...
        xm.set_rng_state(self.states_[name])
...
def model_parallel_xla_manual_seed(seed):
    """Initialize model parallel seed.
    ...
    global _XLA_RNG_STATE_TRACKER
    if _XLA_RNG_STATE_TRACKER is None:
        _XLA_RNG_STATE_TRACKER = XlaRNGStatesTracker()
    _XLA_RNG_STATE_TRACKER.reset()
    # Set the default state.
    import torch_xla.core.xla_model as xm
    xm.set_rng_state(data_parallel_seed)
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, distribute_saved_activations, *args):
        ...
        import torch_xla.core.xla_model as xm
        ctx.fwd_xla_rng_state = xm.get_rng_state()
        ctx.fwd_xla_rng_state_tracker = get_xla_rng_tracker().get_states()
    ...
    def backward(ctx, *args):
        ...
        import torch_xla.core.xla_model as xm
        bwd_xla_rng_state = xm.get_rng_state()
        bwd_xla_rng_state_tracker = get_xla_rng_tracker().get_states()

        # Set the states to what it used to be before the forward pass.
        torch.set_rng_state(ctx.fwd_cpu_rng_state)
        xm.set_rng_state(ctx.fwd_xla_rng_state)
        get_xla_rng_tracker().set_states(ctx.fwd_xla_rng_state_tracker)
        ...
        xm.set_rng_state(bwd_xla_rng_state)
        get_xla_rng_tracker().set_states(bwd_xla_rng_state_tracker)
