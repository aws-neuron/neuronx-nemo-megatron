...
LOG_FILE_NAME="run_gpt_neox_6p7B_seq_len_"$SEQ_LENGTH"_BS_"$UBS"_GBS_"$GBS"_TP_"$TP"_PP_"$PP"_"$(date +"%m-%d-%Y")_$(date +"%H:%M:%S").txt
if [ "$NEURON_EXTRACT_GRAPHS_ONLY" = "1" ]; then
    export TRAIN_ITERS=3
    CREATE_TB_LOGGER=False
    LOG_FILE_NAME="compile_gpt_neox_6p7B_seq_len_"$SEQ_LENGTH"_BS_"$UBS"_GBS_"$GBS"_TP_"$TP"_PP_"$PP"_"$(date +"%m-%d-%Y")_$(date +"%H:%M:%S").txt
