...
def get_checkpoints_for_pp(pp: int, path_to_checkpoints: str, PP: int=1, TP: int=1, is_xser: bool=False):
    ...
    max_step_recorded=max(
        [int(re.match(r".*megatron_neox--step=(\d+).*ckpt$", i).group(1)) 
        for i in glob(template)])
    template = join(path_to_checkpoints, pp_str, f'*megatron_neox--step={max_step_recorded}*.ckpt')
...
def convert_checkpoint(config_file,
    ...
    translation = {
        "embedding.word_embeddings.weight": (1, "gpt_neox.embed_in.weight", 0, 0), # a['model']['language_model']['word_embeddings']['weight']
        ...
        "final_layernorm.weight": (0, "gpt_neox.final_layer_norm.weight", None, 0),
        "output_layer.weight": (1, "embed_out.weight", 0, 0),  # this is shared
    }
    if use_bias: # bias not split across TP ranks
        ...
        "final_layernorm.bias": (0, "gpt_neox.final_layer_norm.bias", None, 0),
        }
        )
