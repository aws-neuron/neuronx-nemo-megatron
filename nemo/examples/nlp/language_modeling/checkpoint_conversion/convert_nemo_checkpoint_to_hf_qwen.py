...
def fix_query_key_value_ordering(param, num_heads, hidden_size_per_head):
    input_shape = param.size()
    saved_shape = (num_heads, 3, hidden_size_per_head) + input_shape[1:]
...
def convert_checkpoint(config_file,
    ...
    translation = {
        "embedding.word_embeddings.weight": (1, "transformer.wte.weight", 0, 0), # a['model']['language_model']['word_embeddings']['weight']
    ...
    for pp in range(PP):
        ...
        for k in layer_keys:
            ...
            if "query_key_value" in k:
                hf_model[hf_key] = fix_query_key_value_ordering(hf_model[hf_key], heads, hidden_size_per_head)
    ...
    if args.bf16:
        for k, v in hf_model.items():
            hf_model[k] = v.to(torch.bfloat16)
...
if __name__ == "__main__":
    ...
    convert_checkpoint(args.config_file, args.path_to_checkpoints, args.output_path, args.checkpoint_version, args.is_xser, args.bf16)
