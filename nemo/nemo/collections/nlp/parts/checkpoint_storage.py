...
class BaseCheckpointStorage:
    ...
    def is_checkpoint_xser(self, dirname: str):
        dirs = self.find_subdirs_contain_path(pattern="*.tensors", search_depth=2, search_root=dirname, max_count=1, sort_by_mdate=False)
        return len(dirs) > 0
    ...
    def find_subdirs_contain_path(self, pattern: str, search_depth: int, search_root: str=None, max_count: int = None, sort_by_mdate: bool=False):
    ...
    @abstractmethod
    def find_files(self, pattern: str, search_depth: int, search_root: str=None, max_count: int = None, sort_by_mdate: bool=False):
        raise NotImplementedError
    ...
    @abstractmethod
    def create_shared_dir(self, dirname: str, exist_ok: bool=True,
...
class S3CheckpointStorage(BaseCheckpointStorage):
            ...
            try:
                ...
                if S3CheckpointStorage.is_slow_down_error(e):
                    if try_idx < max_try - 1:
                        ...
                        logging.info(f"Encountered slow down error when upload file for {try_idx+1} times. Sleep {current_sleep_time} seconds then retry")
                        time.sleep(current_sleep_time)
                        sleep_second *= 1.5
                    else:
                        logging.info(f"Encountered slow down error when upload file for {try_idx+1} times. No more retrying")
                        raise e
                else:
                    raise e
    ...
    def upload_stream_to_file(self, stream_creator, filename: str, chunk_size_MB: int = 64, max_concurrency: int = 10) -> None:
        ...
        key = self.convert_path_to_key(filename)
        S3CheckpointStorage.transfer_fileobj_with_retry(S3CheckpointStorage.UPLOAD, client, self._bucket, key, config, upload_stream_creator=stream_creator)
    ...
    def download_file_to_stream(self, filename: str, chunk_size_MB: int = 64, max_concurrency: int = 15) -> BytesIO:
        ...
        chunk_size = chunk_size_MB * 1048576
        config = boto3.s3.transfer.TransferConfig(multipart_chunksize=chunk_size, max_concurrency=max_concurrency)
        return S3CheckpointStorage.transfer_fileobj_with_retry(S3CheckpointStorage.DOWNLOAD, client, self._bucket, key, config)
    ...
    @staticmethod
    def transfer_fileobj_with_retry(action, client, bucket, key, config, upload_stream_creator = None):
        max_try = 12
        sleep_second = 60
        for try_idx in range(max_try):
            try:
                assert action in [S3CheckpointStorage.DOWNLOAD, S3CheckpointStorage.UPLOAD]
